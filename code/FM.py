...
class FM(BaseEstimator, TransformerMixin):
    ...
    def evaluate_old(self, data):  # evaluate the results for an input set
    ...
    def save_trian_result(self, file_name, first_line):
        ...
        for i in range(len(self.train_rmse)):
            line = "Epoch %d: train=%.4f, validation=%.4f, Test=%.4f " % (
                i + 1, self.train_rmse[i], self.valid_rmse[i], self.test_rmse[i])
